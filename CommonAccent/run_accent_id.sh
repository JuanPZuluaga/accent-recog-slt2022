...
output_dir="accent_id/results/W2V2/"

# data folder:
csv_prepared_folder="accent_id/data"

# If augmentation is defined:
if [ ! "$apply_augmentation" == 'True' ]; then
    output_folder="$output_dir/$(basename $model)-augmented/$seed"
    rir_folder="accent_id/data/rir_folder/"
else
    output_folder="$output_dir/$(basename $model)/$seed"
    rir_folder=""
fi

# configure a GPU to use if we a defined 'CMD'
if [ ! "$cmd" == 'none' ]; then
  basename=train_$(basename $model)_${apply_augmentation}_augmentation
